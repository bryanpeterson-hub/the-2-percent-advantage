...
/** Same gradient frame as “The Playbook” for every pillar. */
const pillarFrameGradient = "from-white/30 via-sf-blue/25 to-sf-blue/5";
...
function PillarCta({
  ...
  const touchCta =
    "touch-manipulation min-h-12 rounded-lg py-3 text-base sm:min-h-9 sm:py-2 sm:text-sm";
  ...
  if (pillar.ctaVariant === "modal") {
    return (
      <Button
        type="button"
        variant="outline"
        className={cn("mt-auto w-full gap-2 shadow-none", touchCta, btnClass)}
  ...
  if (pillar.ctaVariant === "internal" && pillar.href) {
    return (
      <Button variant="outline" className={cn("mt-auto w-full gap-2 shadow-none", touchCta, btnClass)} asChild>
        ...
        {pillars.map((pillar) => {
          ...
          return (
            <div
              key={pillar.key}
              className={cn(
                "group relative flex min-h-[280px] flex-col rounded-2xl p-px transition duration-300 ease-out will-change-transform sm:min-h-[300px]",
            ...
            >
              ...
              />
              <div className="relative flex h-full min-h-[inherit] flex-col rounded-[15px] border border-white/[0.08] bg-black/40 p-5 backdrop-blur-md sm:p-6">
