...
const sheetVariants = cva(
  "fixed z-50 gap-4 glass-panel p-6 shadow-lg transition-transform duration-300 ease-out",
  {
    variants: {
      side: {
...
>(({ side = "left", className, children, ...props }, ref) => (
  <SheetPortal>
    ...
    <SheetPrimitive.Content ref={ref} className={cn(sheetVariants({ side }), className)} {...props}>
      {children}
      <SheetPrimitive.Close className="absolute right-4 top-4 rounded-sm opacity-70 transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-white/30 disabled:pointer-events-none">
        <X className="h-4 w-4" />
